...
const { getUser, postUser, putUser, deleteUser } = require('../services/user.service')

const register = async (req, res, next) => {
	const payload = req.body
	try {
		let user = await postUser(payload)
		return res.status(200).json({ status: 200, data: user, message: "Succesfully User Registered" })
	} catch(e) {
...
const login = async (req, res, next) => {
	passport.authenticate('local', async function(e, user) {
		try {
		  ...
		  await putUser(user._id, {$push: {token: signed}})
			return res.status(200).json({ status: 200, data: user, signed, message: "Succesfully User Logged In" })
		} catch(e) {
			return res.status(400).json({ status: 400, message: e.message });
		}
	})(req, res, next)
...
const logout = async (req, res, next) => {
	...
	try {
		...
		return res.status(200).json({ status: 200, message: "Succesfully User Logged Out" });
	} catch(e) {
		return res.status(400).json({ status: 400, message: e.message });
	}
}
